...
namespace Destructible2D
{
    ...
    public static class D2dTrim
    {
        ...
        public static void Trim(D2dDestructible destructible)
        {
            ...
            {
                if (FastSolidAlphaVertical(yMin, yMax, x, alphaWidth) == false) xMin += 1;
                else break;
            }

            for (var x = xMax - 1; x >= xMin; x--)
            {
                if (FastSolidAlphaVertical(yMin, yMax, x, alphaWidth) == false) xMax -= 1;
                else break;
            }

            for (var y = yMin; y < yMax; y++)
            {
                if (FastSolidAlphaHorizontal(xMin, xMax, y, alphaWidth) == false) yMin += 1;
                else break;
            }

            for (var y = yMax - 1; y >= yMin; y--)
            {
                if (FastSolidAlphaHorizontal(xMin, xMax, y, alphaWidth) == false) yMax -= 1;
                else break;
            }
