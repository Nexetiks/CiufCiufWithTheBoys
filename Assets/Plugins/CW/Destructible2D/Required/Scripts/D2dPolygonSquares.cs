    ...
    public static class D2dPolygonSquares
    {
        ...
        public static void CalculateCells()
        {
            ...
            for (var y = MinY - 1; y <= MaxY; y++)
            {
                ...
                var bl = GetPolyAlpha(0, y);
                var useBl = bl >= 128;
                var tl = GetPolyAlpha(0, y + 1);
                var useTl = tl >= 128;
        ...
        {
            points.Add(square.PointL);
            squareIndex -= 1;
            squareSide = Side.Right;
        }

        private static void SubmitR()
        {
            points.Add(square.PointR);
            squareIndex += 1;
            squareSide = Side.Left;
        }

        private static void SubmitB()
        {
            points.Add(square.PointB);
            squareIndex -= squareWidth;
            squareSide = Side.Top;
        }

        private static void SubmitT()
        {
            points.Add(square.PointT);
            squareIndex += squareWidth;
            squareSide = Side.Bottom;
        }
        ...
        private static D2dPolygonCollider.Path GetNextPath()
        {
            ...
            {
                path = new D2dPolygonCollider.Path();
                paths.Add(path);
            }
            else
            {
